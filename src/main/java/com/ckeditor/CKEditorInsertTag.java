...
public class CKEditorInsertTag extends CKEditorTag {
	...
	/**
	 * Creates the JavaScript {@code replace} (default setting) or {@code inline}
	 * creation method based on the {@code inline} flag. This method also creates
	 * the custom configuration of the editor instance provided that the {@code config}
	 * parameter is not {@code null}.
	 * 
	 * @param config
	 *            the {@code CKEditorConfig} object used to extend the configuration of
	...
	protected String getTagOutput( final CKEditorConfig config ) {
		...
		if ( pageContext.getAttribute( "ckeditor_disable_auto_inline_set" ) == null
			...
			pageContext.setAttribute( "ckeditor_disable_auto_inline_set", new Boolean(
					true ) );
		}
	 ...
	 * website.<br>
	 * Please note, however, that attributes useful for CKEditor or that have any
	 * influence on it are {@code rows}, {@code cols},
	 ...
	 * 
	 * @param inline
	 *            a {@code Boolean} flag indicating which type of editor should be
	 *            inserted into JSP.
	 */
