...
public abstract class CKEditorTag extends TagSupport {
	 ...
	 * Private logger.
	 */
	private static final Logger ckeditorTagLogger = Logger
			.getLogger(CKEditorTag.class.getName());
	/**
	 * Serial identifier.
	 */
	...
	public int doEndTag() {
		...
		try {
			...
			ckeditorTagLogger.log(Level.SEVERE,
					"Could not create CKEditorTag.", ie);
			try {
				HttpServletResponse resp = (HttpServletResponse) pageContext
						.getResponse();
				resp.reset();
				resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR,
						"There was a problem with CKEditorTag creation. Please check server logs.");
	...
	private boolean isInitializedParam() {
		if (pageContext.getAttribute("ckeditor_initialized") != null
				|| pageContext.getRequest()
						.getAttribute("ckeditor_initialized") != null) {
			return true;
		} else {
	 ...
	 * 
	 * @param config
	 *            the {@code CKEditorConfig} object storing configuration for
	 *            the editor instance.
	 ...
	 * <li>{@code window.CKEDITOR.timestamp} if a timestamp string was provided.
	 * </li>
	 * </ul>
	 * <p>
	 * 
	 ...
	 * 
	 * @param config
	 *            the {@code CKEditorConfig} configuration object storing the
