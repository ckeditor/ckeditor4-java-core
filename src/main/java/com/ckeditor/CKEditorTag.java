...
public abstract class CKEditorTag extends TagSupport {
	 ...
	 * path is used, the {@code CKEDITOR_BASEPATH} global variable will be inserted
	 * into JSP.
	 */
	 ...
	 * on JSP (page or request scope), all editors available for the user
	 * during the session (session scope) or all editor instances in an application
	 * (application scope).
	 */
	private GlobalEventHandler globalEvents;

	/**
	 * Creates the {@code CKEditorTag} objects and sets all properties to their default
	 * values.
	 */
	public CKEditorTag ( ) {
		timestamp = null;
	...
	/**
	 * Returns the global configuration object if it is found in the page, request,
	 * session or application scope. The object is searched under
	 * the {@code ckeditor_global_config} name.
	 * 
	 * @return the {@code CKEditorConfig} object if it is found in one of the scopes,
	 *         {@code null} otherwise.
	 * */
	...
	/**
	 * Gets the file browser-specific configuration parameters from {@code pageContext}
	 * and puts them into the tag's {@code CKEditorConfig} object. This method is used
	 * for example in the CKFinder integration.
	 * <p>
	 * <strong>How it works:</strong><br>
	 * CKFinder or any other file browser puts the {@code filebrowserBrowseXYZUrl}
	 * and {@code filebrowserUploadXYZUrl} properties into a {@code Map} which is
	 * then put into another {@code Map}. The second {@code Map} is put in
	 * the {@code pageContext} scope under the attribute named {@code ckeditor-params}.
	 * 
	 * <pre>
	...
	/**
	 * Rewrites configuration options from the {@code Map} provided as a parameter
	 * into the tag's {@code CKEditorConfig} object.
	 * 
	 * @param map
	 *            a {@code Map} with attributes to be written into the tag's
	 ...
	 * {@code ckeditor_initialized} attribute. If this attribute is present in the
	 * page or request scope, it means that the editor was initialized, so
	 * {@code true} is returned. If the attribute is not present, this
	 * method sets it in the page scope and returns {@code false}.
	 * 
	...
	/**
	 * Returns the standard tag output which is an appropriate CKEditor JavaScript
	 * creation method (based on the class implementing it) and in some cases
	 * HTML.
	 * 
	 * @param config
	 *            the {@code CKEditorConfig} object storing configuration for the
	 *            editor instance.
	 ...
	 * <p>
	 * 
	 * The CKEditor tag inserts base parameters by default so there is no need to
	 * do it manually. What is more, the CKEditor tag makes sure that only one set of
	 * these parameters is inserted into JSP.<br>
	 * If, however, the developer wishes to insert base parameters manually or with
	 * a script loading library, the CKEditor tag should be informed about it.
	 * There are two <strong>independent</strong> ways to achieve that:
	 * <ol>
	 * <li>
	 * <strong>With page scope attribute.</strong><br>
	 * The CKEditor tag inserts base parameters during its initialization if
	 * the {@code ckeditor_initialized} attribute is not present in the {@code pageContext}
	 * or {@code request} scope. After inserting parameters CKEditor sets
	 * {@code ckeditor_initialized} in the {@code pageContext} scope to inform other
	 * editor instances that initialization is done.<br>
	 * Developer can set this attribute in page or request scope by putting
	 * appropriate code into JSP (for example with the {@code c:set} tag from the
	 * JSTL library) before the first CKEditor tag.<br>
	...
	/**
	 * Returns the full or relative path to the CKEditor installation directory. If
	 ...
	 * variable will be inserted into JSP.
	 * <p>
	 * If slash ("/") is not the last character of the base path, this method will
	 * add it.
	 * 
	 * @param basePath
	...
	/**
	 * Returns the timestamp value which is used to build the URL for all resources
	...
	/**
	 * Sets a flag informing the CKEditor tag whether base parameters like
	 * the {@code ckeditor.js} script were already inserted into JSP.
	 * 
	 * @param initialized
	 *            the Boolean flag informing whether CKEditor was initialized
	 ...
	 * Depending on scope, these events might be assigned to all editors present
	 * on JSP (page or request scope), all editors available for the user
	 * during the session (session scope) or all editor instances in
	 * the application (application scope).
	 * 
	...
	/**
	 * Returns the CKEditor configuration object which stores the entire editor instance
	 * configuration.
	 * 
	 ...
	 * 
	 * @param config
	 *            the {@code CKEditorConfig} configuration object storing
	 *            the entire editor instance configuration.
	 */
